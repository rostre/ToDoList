...
class CategoryAdapter(val onClickListener: OnClickListener)
    ...
    class CategoryViewHolder(val binding: CategoryListItemBinding) : RecyclerView.ViewHolder(binding.root){
        ...
        fun setupTasks(tasks: List<Todo>){
            val allTasks =
                tasks.filter { it.categoryName == binding.category?.name }.count()
            val completedTasks =
                tasks.filter { it.categoryName == binding.category?.name && it.completed }.count()
            ...
            binding.tvTasksCompleted.text = "$completedTasks completed"
            binding.tvTasksCompleted.setTextColor(color)
            if (leftTasks > 0 ){
                binding.tvTasksLeft.visibility = View.VISIBLE
                binding.tvTasksLeft.text = "${leftTasks} left"
            } else
